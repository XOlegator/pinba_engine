...
static inline size_t pinba_copy_truncated(char *buf, size_t buf_size, const char *str,
                                          size_t str_len) {
  if (!buf || buf_size == 0) {
    return 0;
  }
...
static inline void *pinba_realloc_array_or_log(void *ptr, size_t count, size_t elem_size,
                                               const char *what) {
  return pinba_realloc_or_log(ptr, count * elem_size, what);
}

static inline int pinba_ensure_reports_job_capacity(struct reports_job_data **buffer,
                                                    unsigned int *allocated, size_t required,
...
static inline size_t pinba_append_truncated(char *buf, size_t buf_size, size_t plus,
...
#define memcpy_static(buf, str, str_len, result_len)                   \
  do {                                                                 \
    result_len = pinba_copy_truncated(buf, sizeof(buf), str, str_len); \
  } while (0)

#define memcat_static(buf, plus, str, str_len, result_len)                     \
  do {                                                                         \
    result_len = pinba_append_truncated(buf, sizeof(buf), plus, str, str_len); \
  } while (0)
...
#define timeval_to_pinba_timeval(tv, ptv) \
  {                                       \
...
#define record_get_timer(pool, record, i)                                 \
  (((record->timers_start + i) >= (pool)->size)                           \
       ? (TIMER_POOL((pool)) + (record->timers_start + i - (pool)->size)) \
       : (TIMER_POOL((pool)) + (record->timers_start + i)))
#define record_get_timer_id(pool, record, i)                                                 \
  ((record->timers_start + i) >= (pool)->size) ? ((record->timers_start + i) - (pool)->size) \
                                               : ((record->timers_start + i))

#define CHECK_REPORT_CONDITIONS_CONTINUE(report, record)                                   \
  if (report->flags & PINBA_REPORT_CONDITIONAL) {                                          \
    if (report->cond.min_time > 0.0 &&                                                     \
        timeval_to_float(record->data.req_time) < report->cond.min_time) {                 \
      continue;                                                                            \
    }                                                                                      \
    if (report->cond.max_time > 0.0 &&                                                     \
        timeval_to_float(record->data.req_time) > report->cond.max_time) {                 \
      continue;                                                                            \
    }                                                                                      \
  }                                                                                        \
...
static inline void pinba_update_histogram(pinba_std_report *report, void **histogram_data,
...
#define PINBA_UPDATE_HISTOGRAM_ADD(report, data, value) \
  pinba_update_histogram((pinba_std_report *)(report), &(data), &(value), 1);
#define PINBA_UPDATE_HISTOGRAM_DEL(report, data, value) \
  pinba_update_histogram((pinba_std_report *)(report), &(data), &(value), -1);
#define PINBA_UPDATE_HISTOGRAM_ADD_EX(report, data, value, cnt) \
  pinba_update_histogram((pinba_std_report *)(report), &(data), &(value), (cnt));
#define PINBA_UPDATE_HISTOGRAM_DEL_EX(report, data, value, cnt) \
  pinba_update_histogram((pinba_std_report *)(report), &(data), &(value), -(cnt));

#define PINBA_REPORT_DELETE_CHECK(report, record)                   \
  if (pinba_timercmp(&(report)->std.start, &(record)->time, >) ||   \
      (pinba_timercmp(&(report)->std.start, &(record)->time, ==) && \
       (report)->std.request_pool_start_id > (record)->counter)) {  \
    return;                                                         \
  }
