...
class PinbaException : public std::exception {
 public:
  ...
  std::error_code error_code() const { return make_error_code(code_); }
...
class PinbaSystemException : public PinbaException {
 public:
  ...
  int system_error() const noexcept { return errno_value_; }
...
class Result {
 public:
  ...
  bool is_success() const { return value_ != nullptr; }
