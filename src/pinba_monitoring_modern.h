...
class CounterMetric : public Metric {
 public:
  ...
  void increment(std::int64_t amount = 1) { value_.fetch_add(amount, std::memory_order_relaxed); }
...
class GaugeMetric : public Metric {
 public:
  ...
  void set(std::int64_t val) { value_.store(val, std::memory_order_relaxed); }

  void increment(std::int64_t amount = 1) { value_.fetch_add(amount, std::memory_order_relaxed); }

  void decrement(std::int64_t amount = 1) { value_.fetch_sub(amount, std::memory_order_relaxed); }
