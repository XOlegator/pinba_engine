...
**Action:** После rejection-писем от Launchpad для `2.2.0-2~noble1` workflow изменён так,
чтобы ручной upload не мог собирать релизную версию из уже изменившегося `master`.

**Key finding documented:**
- Если `workflow_dispatch` объявляет `upstream_version=2.2.0`, но `git archive` берёт
  содержимое более позднего commit в `master`, то `orig.tar.gz` и `debian.tar.*`
  перестают соответствовать уже загруженной версии. Launchpad корректно отвергает такой
  upload как reuse того же version number с другими contents.

---

## 2026-06-06 — Launchpad Recovery: Burned upstream orig version in PPA

**Action:** После фикса `source_ref` Launchpad всё равно отвергал `2.2.0-3~*`, потому что
в PPA уже оказался ранее загруженный `pinba-engine_2.2.0.orig.tar.gz` с другими contents.

**Key finding documented:**
- Если в PPA уже существует `orig.tar.gz` для upstream version `2.2.0`, но его contents
  отличаются от правильного source tree, то сам upstream namespace `2.2.0` в этой PPA
  фактически испорчен для дальнейших corrected uploads.
- Практически правильный путь восстановления для этого проекта оказался проще: выпустить
  новый upstream release и использовать его как новый source namespace. Это подтвердил
  release `v2.2.1`, который Launchpad принял:
